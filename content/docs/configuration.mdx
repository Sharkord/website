...
| `trustedProxies`      | empty    | Addresses or CIDR ranges of your reverse proxies. See [Running behind a proxy](#running-behind-a-proxy). |
...
```bash
SHARKORD_TRUSTED_PROXIES="127.0.0.1,172.16.0.0/12"
```
...
When Sharkord sits behind Caddy, Nginx, Apache, or a Docker network, every request arrives from the proxy's address. Anything limited per IP, such as connecting, logging in, joining, and uploading files, then lands in a single shared bucket, and one busy user locks out everyone else.

Listing your proxy in `trustedProxies` fixes that: Sharkord then reads the real client address from the `X-Forwarded-For` or `Forwarded` header the proxy sets.
...
Only list addresses you control. Anything listed here is allowed to claim, on behalf of a client, an address it was not connected from. If the list is empty, Sharkord ignores those headers completely and uses the socket address, which over-limits rather than letting anyone spoof their way around a limit.

See the [HTTPS setup guides](/docs/https/why) for full reverse proxy examples.
