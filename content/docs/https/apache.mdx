...
Apache forwards the client address in `X-Forwarded-For`, but Sharkord ignores that header unless it knows the request came from your proxy. Without this, every user shares one rate limit bucket.
